...
const report={phase:'1B.3',complete:false,localOnly:true,rawOutputPersisted:false,remoteWrites:0,productionRequests:0,suites:[],startedAt:new Date().toISOString()};
...
const syntheticEnv={...baseEnv,NODE_ENV:'test',NEXT_PUBLIC_SUPABASE_URL:'http://127.0.0.1:54321',NEXT_PUBLIC_SUPABASE_ANON_KEY:'synthetic-anon',
  SUPABASE_SERVICE_ROLE_KEY:'phase1b3-local-service-canary-0000000000000000',REVALIDATION_ENV:'local',REVALIDATION_LOCAL_SYNTHETIC:'1'};
...
async function run(name,args,{cwd=root,env=baseEnv,kind='command'}={}) {
  ...
  if(kind==='database') {
    const data=JSON.parse(readFileSync(join(folder,'database-validation.json')));
...
try {
  const database=JSON.parse(readFileSync(join(folder,'database-validation.json')));
  const pending=JSON.parse(readFileSync(join(folder,'pending-validation.json')));
  if(!pending.complete||database.migrations.length!==271)throw Error('Database verification is not complete; do not run later suites');
  report.databaseEvidence={reusedFile:'docs/security/phase-1b3/database-validation.json',pendingFile:'docs/security/phase-1b3/pending-validation.json',reusedPassed:database.passed,pendingPassed:pending.passed,migrations:271};
  const frontend=join(root,'frontend');
  await run('phone-frontend-and-public-handler-contracts',['--import','tsx','--test','--test-concurrency=1',...['phoneAdministrationClient','phase0PhoneHandlers','landingPhoneNormalization','whatsappDisplayPhone'].map(n=>'tests/'+n+'.test.ts')],{cwd:frontend,env:syntheticEnv,kind:'tap'});
  ...
  const build=mkdtempSync(join(tmpdir(),'phase1b3-build-'));
  ...
  const forbidden=['SUPABASE_SERVICE_ROLE_KEY','SERVICE_ROLE_KEY','phase1b3-local-service-canary-0000000000000000','phase1b3-local-only-synthetic-0000000000000000'];
  ...
  report.bundle={scope:'.next/static browser assets',excludedServerArtifacts:'.next/server',files:chunks.length,findings,secretValuesUsed:'synthetic only',browserContainsPrivateCredential:findings.length>0};save();
