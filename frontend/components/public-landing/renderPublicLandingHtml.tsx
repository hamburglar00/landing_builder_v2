...
function renderTemplate4({ config }: RenderParams) {
  ...
  const backgroundStyle = backgroundImageUrl
    ? ` style="--template4-chat-background-image:url(&quot;${escapeHtml(backgroundImageUrl)}&quot;);--template4-chat-background-size:cover;--template4-chat-background-position:center;--template4-chat-background-opacity:1;--template4-chat-background-overlay:linear-gradient(rgba(7,16,19,.42),rgba(7,16,19,.42))"`
    : "";
  ...
  const ctaText = config.content?.ctaText?.trim() || "ABRIR WHATSAPP";
  const ctaColor = config.colors?.ctaText || "#FFFFFF";
  const ctaBackground = config.colors?.ctaBackground || "#25D366";
...
function renderTemplate5({ config }: RenderParams) {
  ...
  return `<main class="public-landing template5"><section class="template5__phone" aria-label="Atencion en vivo"><div class="template5__curtain"><span>EN VIVO</span><strong>Entrando...</strong></div><div class="template5__topline"><div class="template5__live-badge"><span class="template5__live-dot"></span><strong>EN VIVO</strong><time>${currentTime}</time></div><span class="template5__viewers">1.278 viendo</span></div><section class="template5__hero" data-public-landing-trigger><h1><span>ESTA PASANDO</span><b>AHORA MISMO.</b></h1><p>Un asesor te abre la cuenta en 2 minutos por WhatsApp y te acompaña en todo el proceso...</p></section><section class="template5__advisor"><div class="template5__avatar" aria-hidden="true">foto<br>asesor</div><div><strong>${name} · tu asesora</strong><span>En linea · responde en ~40 seg</span></div><i></i></section><div class="template5__progress" aria-hidden="true"><span></span></div><section class="template5__feed" aria-label="Retiros pagados en vivo"><div><strong><span class="template5__feed-dot"></span> RETIROS PAGADOS · EN VIVO</strong></div><p><span><b>Camilo A.</b><small>hace 5 s</small></span><strong>$ 1.150.000</strong></p><p><span><b>Sebastian G.</b><small>hace 17 s</small></span><strong>$ 260.000</strong></p><p><span><b>Laura P.</b><small>hace 29 s</small></span><strong>$ 780.000</strong></p></section><footer class="template5__footer"><button type="button" class="template5__cta" data-public-landing-cta data-public-landing-rest-label="ENTRAR POR WHATSAPP" data-public-landing-loading-label="Abriendo..." data-public-landing-disabled-label="Sin numero disponible" aria-label="ENTRAR POR WHATSAPP">${renderWhatsAppIcon("template5__cta-icon")}<span data-public-landing-cta-label>ENTRAR POR WHATSAPP</span></button><small>${name} te contesta en persona, ahora mismo</small></footer></section></main>`;
}
