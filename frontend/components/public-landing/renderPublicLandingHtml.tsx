...
function buildPreloadLinks(config: PublicLandingConfig) {
  ...
  return [
    ...
    `<link rel="preload" as="image" href="/whatsapp-icon.png">`,
  ]
    .filter(Boolean)
    .join("");
}
...
function renderWhatsAppButton(
  ...
  return `<button type="button" class="${buttonClass}"${styleAttr(buttonStyle)} data-public-landing-cta${attr(
    ...
    options.hideButton
      ? ""
      : `<img src="/whatsapp-icon.png" alt="" class="${iconClass}" width="24" height="24" decoding="async">`
  }</button>`;
}
...
export function renderPublicLandingHtml(params: RenderParams) {
  ...
  return `<!doctype html><html lang="es"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover"><title>${escapeHtml(
    ...
    description,
  )}"><meta name="theme-color" content="#000000"><link rel="manifest" href="/manifest.webmanifest"><link rel="icon" href="/favicon.ico"><link rel="icon" href="/favicon-16x16.png" sizes="16x16" type="image/png"><link rel="icon" href="/favicon-32x32.png" sizes="32x32" type="image/png"><link rel="apple-touch-icon" href="/apple-touch-icon.png" sizes="180x180" type="image/png">${
    supabaseOrigin
