...
const DASHBOARD_PREFETCH_ROUTES = [
  ...
  "/dashboard/whatsapp-cloud-api/logs",
  "/dashboard/gerencias",
  "/dashboard/telefonos",
    ...
    return (
      <span className={`${base}`}>
        <svg
          ...
          strokeLinejoin="round"
        >
          <path d="M15 17h5l-1.4-1.4a2 2 0 0 1-.6-1.4V10a6 6 0 1 0-12 0v4.2a2 2 0 0 1-.6 1.4L4 17h5" className={accent} />
...
export default function DashboardLayout({
  ...
  const [planMaxLandings, setPlanMaxLandings] = useState<number>(1);
  ...
  useEffect(() => {
    const checkSession = async () => {
      ...
      setPlanMaxLandings(Number(sub?.max_landings ?? 1));
    ...
    <div className="panel-shell flex min-h-screen text-[var(--color-text)]">
      ...
      >
        ...
        <nav className="panel-sidebar-nav flex flex-1 flex-col gap-1 overflow-y-auto p-3">
          ...
          <Link
            href="/dashboard/whatsapp-cloud-api"
            onClick={(event) => handleNavClick("/dashboard/whatsapp-cloud-api", event)}
            className={`flex items-center gap-2 rounded-lg px-3 py-2 text-xs font-medium tracking-[0.18em] transition ${
              pathname?.startsWith("/dashboard/whatsapp-cloud-api")
                ? "bg-[var(--color-primary-soft-bg)] text-[var(--color-primary)] border border-[var(--color-primary-soft-border)]"
                : "text-[var(--color-text-muted)] hover:bg-[var(--color-bg-3)] hover:text-[var(--color-text)]"
            }`}
          >
            <NavIcon
              variant="whatsapp"
              active={Boolean(pathname?.startsWith("/dashboard/whatsapp-cloud-api"))}
            />
          ...
          >
            <NavIcon
              ...
              active={Boolean(pathname?.startsWith("/dashboard/notificaciones"))}
            />
