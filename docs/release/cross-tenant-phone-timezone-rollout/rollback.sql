-- Exact pre-279 function definitions captured read-only from production on 2026-09-25.

-- Execute only after restoring Vercel and landing-phone, and only as compensation.
...
CREATE OR REPLACE FUNCTION public.get_phone_for_landing(p_landing_name text, p_create_reservation boolean)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.record_landing_phone_availability_demand(p_landing_name text, p_request_id uuid, p_source text DEFAULT 'landing-phone'::text, p_result_status text DEFAULT 'ok'::text, p_selected_gerencia_id integer DEFAULT NULL::integer, p_selected_phone_id bigint DEFAULT NULL::bigint, p_selected_phone text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
