...
No sumar estos totales como pruebas únicas globales: hay regresiones repetidas entre fases. Las suites de alcance no afectado no se repiten en una auditoría documental. Node 24 y el contrato del código clásico desplegado ya se validaron localmente: 57 tests del candidato y siete de compatibilidad clásica. El ensayo agrega transiciones, handlers y los 36 controles Inbox. Siguen pendientes Edge/Vercel reales, acuerdo de credenciales y smoke productivo autorizado. Ver [rollout-rehearsal.json](rollout-rehearsal.json).
