...
Antes de ejecutar, consultar --help de la CLI, verificar proyecto/ledger y dry-run: deben aparecer exactamente seis versiones. No usar --include-all, db reset, repair, bootstrap local ni cambios de extensiones. Exigir transacciones por migración, límite de espera de lock de 5 segundos y presupuesto acotado de ejecución mediante PGSERVICEFILE/PGSERVICE en Supabase CLI 2.75.0: lock_timeout=5s y statement_timeout=45s. Ese mecanismo y el rollback de archivo/ledger se probaron localmente; PGOPTIONS no acredita límites en esta versión. Procedimiento en [operational-runbook.md](operational-runbook.md). Ante timeout, abortar esa transacción y revisar; no ampliar límites ni cancelar sesiones ajenas automáticamente.
...
Ensayo nuevo: cada una de las seis aplicaciones tardó aproximadamente 1–1,4 segundos de pared local, incluyendo Docker/psql; no es tiempo DDL puro ni una estimación productiva. Dos provisiones llegaron a 274, una para la transición completa y otra para terminar la comparación Inbox tras corregir únicamente el límite del oráculo histórico. Ver [rollout-rehearsal.json](rollout-rehearsal.json).
