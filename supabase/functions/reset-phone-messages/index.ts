...
/**
 * Reset operativo de mensajes recibidos para telefonos del usuario.
 * No toca conversions: solo setea messages_reset_at en gerencia_phones.
 * Body: { user_id: string, gerencia_id?: number }
 */
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!userId) {
      return new Response(
        JSON.stringify({ error: "Falta user_id en el cuerpo de la peticion." }),
        {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        },
    ...
    if (gerenciasError) {
      return new Response(
        JSON.stringify({ error: "Error al obtener las gerencias del usuario." }),
        {
          status: 500,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        },
    ...
    if (phonesError) {
      return new Response(
        JSON.stringify({ error: "Error al obtener los telefonos para resetear." }),
        {
          status: 500,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        },
    ...
    const { error: updateError } = await supabaseAdmin
      .from("gerencia_phones")
      .update({ messages_reset_at: resetAt })
      .in("id", phoneIds);

    if (updateError) {
      return new Response(
        JSON.stringify({ error: "No se pudieron reiniciar los mensajes." }),
        {
          status: 500,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        },
    ...
    console.error(err);
    return new Response(
      JSON.stringify({ error: "Error inesperado al reiniciar mensajes." }),
      {
        status: 500,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      },
