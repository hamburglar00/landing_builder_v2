...
alter table public.conversion_inbox
  drop constraint if exists conversion_inbox_workspace_currency_check;

alter table public.conversion_inbox
  add constraint conversion_inbox_workspace_currency_check
  check (workspace_currency in ('ARS', 'PYG'));

create index if not exists conversion_inbox_user_workspace_created_idx
  on public.conversion_inbox (user_id, workspace_currency, created_at desc);

create index if not exists conversions_user_currency_created_idx
  on public.conversions (user_id, currency, created_at desc);

-- Canonicaliza filas historicas de conversions con senales internas fuertes.
update public.conversions c
set currency = l.workspace_currency
from public.landings l
where c.user_id = l.user_id
  and c.landing_id = l.id
  and l.workspace_currency in ('ARS', 'PYG')
  and coalesce(c.currency, '') <> l.workspace_currency;
...
update public.conversions c
set currency = t.workspace_currency
from unique_landing_tags t
where c.user_id = t.user_id
  and split_part(coalesce(c.promo_code, ''), '-', 1) = t.landing_tag
  and coalesce(c.promo_code, '') like '%-%'
  and coalesce(c.currency, '') <> t.workspace_currency;

update public.conversions c
set currency = g.workspace_currency
from public.gerencias g
where c.user_id = g.user_id
  and c.assigned_gerencia_id = g.id
  and c.lead_gerencia_id is null
  and c.purchase_gerencia_id is null
  and g.workspace_currency in ('ARS', 'PYG')
  and coalesce(c.currency, '') <> g.workspace_currency;

update public.conversions c
set currency = g.workspace_currency
from public.gerencias g
where c.user_id = g.user_id
  and c.lead_gerencia_id = g.id
  and c.purchase_gerencia_id is null
  and g.workspace_currency in ('ARS', 'PYG')
  and coalesce(c.currency, '') <> g.workspace_currency;
...
from public.gerencias g
where c.user_id = g.user_id
  and c.purchase_gerencia_id = g.id
  and g.workspace_currency in ('ARS', 'PYG')
  and coalesce(c.currency, '') <> g.workspace_currency;

-- Backfill de inbox: primero por conversion vinculada, despues por promo tag/landing.
...
where i.conversion_id = c.id
  ...
  and i.workspace_currency <> c.currency;
...
update public.conversion_inbox i
set workspace_currency = t.workspace_currency
from unique_landing_tags t
where i.user_id = t.user_id
  and split_part(coalesce(i.promo_code, ''), '-', 1) = t.landing_tag
  and coalesce(i.promo_code, '') like '%-%'
  and i.workspace_currency <> t.workspace_currency;

update public.conversion_inbox i
set workspace_currency = l.workspace_currency
from public.landings l
where i.user_id = l.user_id
  and i.landing_name = l.name
  and l.workspace_currency in ('ARS', 'PYG')
  and i.workspace_currency <> l.workspace_currency;
...
comment on function public.get_whatsapp_cloud_api_inbox_threads(integer, text) is
  'Devuelve threads del Inbox de WhatsApp Cloud API filtrados por workspace cuando p_workspace_currency es ARS/PYG.';
