create table if not exists public.whatsapp_cloud_api_thread_reads (
  ...
  primary key (reader_user_id, contact_id)
);

create index if not exists whatsapp_cloud_api_thread_reads_contact_idx
  on public.whatsapp_cloud_api_thread_reads (contact_id);

drop trigger if exists whatsapp_cloud_api_thread_reads_updated_at on public.whatsapp_cloud_api_thread_reads;
create trigger whatsapp_cloud_api_thread_reads_updated_at
before update on public.whatsapp_cloud_api_thread_reads
for each row execute function public.set_whatsapp_cloud_api_updated_at();

alter table public.whatsapp_cloud_api_thread_reads enable row level security;

drop policy if exists "whatsapp_cloud_api_thread_reads_owner_select" on public.whatsapp_cloud_api_thread_reads;
create policy "whatsapp_cloud_api_thread_reads_owner_select"
on public.whatsapp_cloud_api_thread_reads for select
to authenticated
using ((select auth.uid()) = reader_user_id);

drop policy if exists "whatsapp_cloud_api_thread_reads_owner_insert" on public.whatsapp_cloud_api_thread_reads;
create policy "whatsapp_cloud_api_thread_reads_owner_insert"
on public.whatsapp_cloud_api_thread_reads for insert
to authenticated
with check ((select auth.uid()) = reader_user_id);

drop policy if exists "whatsapp_cloud_api_thread_reads_owner_update" on public.whatsapp_cloud_api_thread_reads;
create policy "whatsapp_cloud_api_thread_reads_owner_update"
on public.whatsapp_cloud_api_thread_reads for update
to authenticated
using ((select auth.uid()) = reader_user_id)
with check ((select auth.uid()) = reader_user_id);

grant select, insert, update on table public.whatsapp_cloud_api_thread_reads to authenticated;
grant all on table public.whatsapp_cloud_api_thread_reads to service_role;

create or replace function public.mark_whatsapp_cloud_api_thread_read(
  p_contact_id uuid
)
returns void
language plpgsql
...
begin
  ...
  select c.config_id, c.user_id, c.wa_id
  into v_config_id, v_contact_owner, v_wa_id
  from public.whatsapp_cloud_api_contacts c
  where c.id = p_contact_id;

  if v_config_id is null then
    raise exception 'Thread no encontrado';
  end if;

  if not v_is_admin and v_contact_owner <> v_uid then
    raise exception 'Sin permiso para marcar este thread';
  end if;

  select e.received_at, coalesce(e.meta_message_id, '')
  into v_last_read_at, v_last_message_id
  from public.whatsapp_cloud_api_webhook_events e
  where e.config_id = v_config_id
  ...
  values (
    ...
    p_contact_id,
    v_config_id,
    coalesce(v_last_read_at, now()),
    coalesce(v_last_message_id, '')
  )
...
revoke all on function public.mark_whatsapp_cloud_api_thread_read(uuid) from public, anon, authenticated;
grant execute on function public.mark_whatsapp_cloud_api_thread_read(uuid) to authenticated;
...
comment on function public.get_whatsapp_cloud_api_inbox_threads(integer, text) is
  'Devuelve threads del Inbox de WhatsApp Cloud API filtrados por workspace, etapa comercial, clicks de redireccion y no leidos por usuario.';
