-- Add historical stage-reached indicators to funnel_contacts VIEW.
-- These use event_ids (which persist even when estado is overwritten)
-- to determine which stages a contact historically reached.
-- Must DROP + CREATE because new columns change column order.
...
select
  ...
  -- Historical stage-reached indicators (based on event_ids, not current estado)
  ...
  bool_or(c.observaciones like '%REPEAT%') as reached_repeat,
...
comment on view public.funnel_contacts is 'Vista agregada por teléfono para funnel y estadísticas. Incluye indicadores históricos de etapa alcanzada.';
