alter table public.whatsapp_cloud_api_configs
  add column if not exists send_business_messaging_purchase_type_capi boolean not null default false;

comment on column public.whatsapp_cloud_api_configs.send_business_messaging_purchase_type_capi is
  'Si true, Purchase de WhatsApp Cloud API agrega custom_data.purchase_type first/repeat en Business Messaging CAPI cuando el dato existe. Default false para mantener el payload minimo documentado.';
