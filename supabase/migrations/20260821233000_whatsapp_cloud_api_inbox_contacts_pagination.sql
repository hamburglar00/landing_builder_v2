...
drop function if exists public.get_whatsapp_cloud_api_inbox_threads(integer);
...
comment on function public.get_whatsapp_cloud_api_inbox_threads_page(integer, integer, text) is
  'Devuelve una pagina de threads del Inbox de WhatsApp Cloud API. Limite maximo 20 para mantener la UI agil.';
...
as $$
  ...
  conversion_metrics as (
    ...
    left join public.conversions c
     ...
     and (
       (pc.external_id <> '' and c.external_id = pc.external_id)
       or (la.promo_code <> '' and c.promo_code = la.promo_code)
       or (la.conversion_id is not null and c.id = la.conversion_id)
     )
     and coalesce(c.test_event_code, '') = ''
  ...
  select
    ...
    pc.wa_id,
    pc.phone,
    coalesce(pc.profile_name, '') as profile_name,
    pc.last_message_at,
    case
      when coalesce(cm.total_loaded, 0) >= coalesce(t.premium_threshold, 50000) then 'premium'
      when coalesce(cm.repeat_purchase_count, 0) > 0 or coalesce(cm.purchase_count, 0) > 1 then 'recompra'
...
comment on function public.get_whatsapp_cloud_api_contacts_page(integer, integer, text) is
  'Devuelve contactos de WhatsApp Cloud API paginados con estado comercial derivado.';
