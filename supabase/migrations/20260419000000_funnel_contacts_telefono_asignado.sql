-- Incluir contactos con phone vacío pero telefono_asignado en funnel_contacts.
-- Usa telefono_asignado como fallback para agrupar cuando phone está vacío.
...
select
  ...
  coalesce(
    nullif(trim(c.phone), ''),
    nullif(trim(c.telefono_asignado), ''),
    '_' || c.id::text
  ) as phone,
...
group by c.user_id, coalesce(
  nullif(trim(c.phone), ''),
  nullif(trim(c.telefono_asignado), ''),
  '_' || c.id::text
);

comment on view public.funnel_contacts is 'Vista agregada por teléfono para funnel y estadísticas. Usa telefono_asignado como fallback cuando phone está vacío.';
