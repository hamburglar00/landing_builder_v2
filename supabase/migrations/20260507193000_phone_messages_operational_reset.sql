-- Reset operativo de mensajes recibidos por telefono.
-- No borra ni modifica conversions: solo guarda desde que momento contar para TELEFONOS
-- y para el criterio fair por messages_received.

alter table public.gerencia_phones
  add column if not exists messages_reset_at timestamptz null;

comment on column public.gerencia_phones.messages_reset_at is
  'Punto de corte operativo para reiniciar mensajes recibidos por telefono sin borrar el historico de conversions.';

create index if not exists conversions_user_assigned_phone_lead_time_idx
  on public.conversions (user_id, telefono_asignado, lead_event_time)
