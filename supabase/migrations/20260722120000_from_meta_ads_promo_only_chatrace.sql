create or replace function public.set_conversions_from_meta_ads()
returns trigger
language plpgsql
as $$
begin
  new.from_meta_ads := (
    coalesce(new.fbc, '') <> ''
    or coalesce(new.utm_campaign, '') <> ''
    or (
      lower(btrim(coalesce(new.source_platform, ''))) = 'chatrace'
      and coalesce(new.promo_code, '') ~ '^[A-Za-z0-9]+-[A-Za-z0-9]+$'
    )
  );
  return new;
end;
$$;

comment on column public.conversions.from_meta_ads is
  'True cuando la conversion trae fbc, utm_campaign, o promo_code valido TAG-SUFIX solo si source_platform es chatrace.';

update public.conversions
set from_meta_ads = (
  coalesce(fbc, '') <> ''
  or coalesce(utm_campaign, '') <> ''
  or (
    lower(btrim(coalesce(source_platform, ''))) = 'chatrace'
    and coalesce(promo_code, '') ~ '^[A-Za-z0-9]+-[A-Za-z0-9]+$'
  )
);
