...
begin
  ...
  non_test_monthly as (
    select *
    from monthly
    where trim(coalesce(test_event_code, '')) = ''
  ),
  grouped_funnel as (
    select
      phone,
  ...
  return jsonb_build_object(
    ...
    'porcentaje_carga', case
      when v_unique_leads > 0 then (v_first_load_linked::numeric / v_unique_leads::numeric) * 100
      else 0
    end,
    'carga_promedio', case
